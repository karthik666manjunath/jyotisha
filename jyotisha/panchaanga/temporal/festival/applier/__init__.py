...
class FestivalAssigner(PeriodicPanchaangaApplier):
  ...
  def assign_sidereal_solar_day_fest(self, fest_rule, d):
    ...
    if self.daily_panchaangas[d].solar_sidereal_date_sunset.month == month_num:
      if self.daily_panchaangas[d].solar_sidereal_date_sunset.day == anga_num:
        if kaala == 'arunodaya':
          prev_angas = self.daily_panchaangas[d-1].day_length_based_periods.arunodaya.get_boundary_angas(anga_type=AngaType.SOLAR_MONTH, ayanaamsha_id=self.ayanaamsha_id).to_tuple()
          current_angas = self.daily_panchaangas[d].day_length_based_periods.arunodaya.get_boundary_angas(anga_type=AngaType.SOLAR_MONTH, ayanaamsha_id=self.ayanaamsha_id).to_tuple()
  ...
  def get_2_day_interval_boundary_angas(self, kaala, anga_type, d):
    if kaala == 'arunodaya':
      ...
      interval_prev = Interval(name=kaala, jd_start=self.daily_panchaangas[d-1].jd_sunrise, jd_end=None)
      interval_next = Interval(name=kaala, jd_start=self.daily_panchaangas[d].jd_sunrise, jd_end=None)
    elif kaala == 'sunrise':
      interval_prev = Interval(name=kaala, jd_start=self.daily_panchaangas[d].jd_sunrise, jd_end=None)
      interval_next = Interval(name=kaala, jd_start=self.daily_panchaangas[d+1].jd_sunrise, jd_end=None)
    elif kaala == 'sunset':
      interval_prev = Interval(name=kaala, jd_start=self.daily_panchaangas[d].jd_sunset, jd_end=None)
      interval_next = Interval(name=kaala, jd_start=self.daily_panchaangas[d+1].jd_sunset, jd_end=None)
    elif kaala == 'moonrise':
      interval_prev = Interval(name=kaala, jd_start=self.daily_panchaangas[d].jd_moonrise, jd_end=None)
      interval_next = Interval(name=kaala, jd_start=self.daily_panchaangas[d+1].jd_moonrise, jd_end=None)
    else:
  ...
  def decide_aparaahna_vyaapti_priority(self, d0_angas, d1_angas, target_anga):
    ...
    if d0_angas.end >= r:
      ...
      fday = 0
    elif d0_angas.end == p and d1_angas.start >= q:
      # Covers p p r r, [p, p, q, r], [p, p, q, q]
      fday = 1
    elif d1_angas.start == q and d1_angas.end == q:
      fday = 1
    elif d0_angas.start == q and d1_angas.start >= r:
      fday = 0
  ...
  def assign_tithi_yoga_nakshatra_fest(self, fest_rule, d):
    ...
    if anga_sunrise[d] == prev_anga or anga_sunrise[d] == target_anga:
      ...
      if priority == 'paraviddha':
        ...
        raise ValueError('Unknown priority "%s" for %s! Check the rules!' % (priority, festival_name))

    if fday is not None:
      if (month_type == 'lunar_month' and ((self.daily_panchaangas[d].lunar_month_sunrise == month_num or month_num == 0) or (
          (self.daily_panchaangas[d + 1].lunar_month_sunrise == month_num and target_anga == 1)))) or \
        ...
        if month_type == 'lunar_month' and target_anga == 1 and self.daily_panchaangas[
